...
# Reading data
data <- read.csv(ADCTLtrain_dir, header = TRUE)
...
rf <- randomForest(labels_factor ~ ., data = train[, c(selected_features, 431)])
...
test_pred <- predict(rf, newdata = test[, c(selected_features, 431)])
